...
async function readAbapGitIgnores(rootDir: string): Promise<string[]> {
  try {
    ...
    const items: unknown =
      parsed?.['asx:abap']?.['asx:values']?.DATA?.IGNORE?.item;
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (
          matchesIncludes(name, relPosix, includes) &&
          !isIgnored(relPosix, ignores)
        ) {
          yield relPosix;
        }
      }
...
function parseDdicXml(
  ...
  const objectKind: 'table' | 'structure' =
    kind === 'tabl' ? 'table' : 'structure';
  // XML hardening: disable entity expansion (CWE-611 / XXE). abapGit
  ...
  const objectName = String(
    dd02v?.TABNAME ?? relPosix.replace(/\.[^.]+$/u, '')
  );
  ...
  const fields = rows
    ...
    .map((row) => {
      ...
      return {
        ...
        ...(rollName.length > 0
          ? { typeRef: { kind: 'data-element', name: rollName } }
          : {}),
...
export async function extract(opts: {
  ...
  const extracted: { rel: string; obj: AbapObject }[] = await extractObjects(
    rootDir,
    includes,
    ignores
  );
  if (extracted.length === 0) {
    warnings.push(
      `No matching ABAP files found under ${rootDir} (includes: ${includes.join(
        ', '
      )})`
    );
  }
